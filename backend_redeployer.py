...
def main():
    ...
    container_id = run_command("docker ps -a -q --filter ancestor=purrytails-backend")
    
    if container_id:
        print(f"Stopping and removing container: {container_id}")
        run_command(f"docker rm -f {container_id}")
    else:
        print("No running container found for 'purrytails-backend'")
    
    # Remove the images
    run_command("docker rmi purrytails-backend")
    run_command("docker rmi abhaymathur21/purrytails:backend")
    ...
    run_command("docker tag purrytails-backend abhaymathur21/purrytails:backend")
    run_command("docker push abhaymathur21/purrytails:backend")
